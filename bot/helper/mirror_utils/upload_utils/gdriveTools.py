...
class GoogleDriveHelper:
    ...
    def clone(self, link, gdrive_id):
        ...
        try:
            ...
            return self.__listener.onDownloadError(
        ...
        try:
            ...
            return
        return self.__listener.onUploadComplete(
            durl,
            size,
            self.__total_files,
    ...
    def drive_list(
            ...
            for file in response.get("files", []):
                ...
                if mime_type == "application/vnd.google-apps.folder":
                    ...
                    if not config_dict["DISABLE_DRIVE_LINK"]:
                        ...
                        if SHORTENERES:
                            msg += (
                                f"⁍<a href='{furl}'>{file.get('name').replace(' ', '-').replace('.', ',')}"
                                f"</a> (shortcut)"
                            )
                        else:
                            msg += (
        ...
        path = [
            telegraph.create_page(title="Jmdkh-mltb Drive Search", content=content)[
                "path"
            ]
