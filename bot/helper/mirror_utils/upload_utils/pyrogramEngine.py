...
class TgUploader:
            ...
            for file_ in sorted(files, key=ospath.splitext):
                try:
                    ...
                    if not self.__is_corrupted and (
                        not self.__listener.isPrivate or config_dict["DUMP_CHAT"]
                    ):
    ...
    def __msg_to_reply(self):
        if DUMP_CHAT := config_dict["DUMP_CHAT"]:
            if self.__listener.logMessage:
                self.__sent_msg = app.copy_message(
                    DUMP_CHAT,
                    self.__listener.logMessage.chat.id,
                    self.__listener.logMessage.message_id,
                )
            else:
                msg = (
                    self.__listener.message.text
                    if self.__listener.isPrivate
                    else f'<b><a href="{self.__listener.message.link}">Source</a></b>'
                )
                msg = f"{msg}\n\n<b>#cc</b>: {self.__listener.tag} (<code>{self.__listener.message.from_user.id}</code>)"
                self.__sent_msg = app.send_message(
                    DUMP_CHAT, msg, disable_web_page_preview=True
                )
            ...
            self.__sent_msg = app.get_messages(
                self.__listener.message.from_user.id,
                self.__listener.dmMessage.message_id,
            )
        else:
            self.__sent_msg = app.get_messages(
        ...
        if (
            not IS_USER_SESSION
            and self.__listener.message.chat.type != "private"
            and config_dict["DUMP_CHAT"]
        ) or not self.__listener.dmMessage:
            self.__button = InlineKeyboardMarkup(
    ...
    def __get_input_media(self, subkey, key):
        ...
        for msg in self.__media_dict[key][subkey]:
            if key == "videos":
                ...
                ptbinput_media = ptbInputMediaVideo(
                    media=msg.video.file_id, caption=msg.caption
                )
            else:
                ...
                ptbinput_media = ptbInputMediaDocument(
                    media=msg.document.file_id, caption=msg.caption
                )
    ...
    def __send_media_group(self, subkey, key, msgs):
        ...
        msgs_list = msgs[0].reply_to_message.reply_media_group(
            media=pyromedia, quote=True, disable_notification=True
        )
        for msg in msgs:
        ...
        if self.__sent_DMmsg:
            msgs_list = self.__sent_DMmsg.reply_media_group(
                ptbmdedia, quote=True, disable_notification=True
            )
