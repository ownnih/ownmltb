...
def bt_selection_buttons(id_: str, isCanCncl: bool = True):
    ...
    if config_dict["WEB_PINCODE"]:
        buttons.buildbutton(
            "Select Files", f"{config_dict['BASE_URL']}/app/files/{id_}"
        )
        buttons.sbutton("Pincode", f"btsel pin {gid} {pincode}")
    else:
        buttons.buildbutton(
            "Select Files",
            f"{config_dict['BASE_URL']}/app/files/{id_}?pin_code={pincode}",
        )
...
def get_category_btns(time_out, msg_id, c_index):
    ...
    for i, _name in enumerate(CATEGORY_NAMES):
        button.sbutton(
            f'{_name}{" ✅" if _name == CATEGORY_NAMES[c_index] else ""}',
            f"change scat {msg_id} {i}",
        )
    button.sbutton("Skip", f"change cancel {msg_id}", "footer")
    button.sbutton(
        f"Done ({get_readable_time(time_out)})", f"change done {msg_id}", "footer"
    )
    ...
    if config_dict["SET_COMMANDS"]:
        bot.set_my_commands(
            [
                ...
                (
                    f"{BotCommands.ZipMirrorCommand[0]}",
                    f"or /{BotCommands.ZipMirrorCommand[1]} Mirror and upload as zip",
                ),
                (
                    f"{BotCommands.ZipLeechCommand[0]}",
                    f"or /{BotCommands.ZipLeechCommand[1]} Leech and upload as zip",
                ),
                (
                    f"{BotCommands.UnzipMirrorCommand[0]}",
                    f"or /{BotCommands.UnzipMirrorCommand[1]} Mirror and extract files",
                ),
                (
                    f"{BotCommands.UnzipLeechCommand[0]}",
                    f"or /{BotCommands.UnzipLeechCommand[1]} Leech and extract files",
                ),
                (
                ...
                (
                    ...
                    f"or /{BotCommands.QbZipMirrorCommand[1]} Mirror torrent and upload as zip using qb",
                ),
                (
                    f"{BotCommands.QbZipLeechCommand[0]}",
                    f"or /{BotCommands.QbZipLeechCommand[1]} Leech torrent and upload as zip using qb",
                ),
                (
                    f"{BotCommands.QbUnzipMirrorCommand[0]}",
                    f"or /{BotCommands.QbUnzipMirrorCommand[1]} Mirror torrent and extract files using qb",
                ),
                (
                    f"{BotCommands.QbUnzipLeechCommand[0]}",
                    f"or /{BotCommands.QbUnzipLeechCommand[1]} Leech torrent and extract using qb",
                ),
                (
                ...
                (
                    f"{BotCommands.YtdlZipCommand[0]}",
                    f"or /{BotCommands.YtdlZipCommand[1]} Mirror yt-dlp supported link as zip",
                ),
                (
                    f"{BotCommands.YtdlZipLeechCommand[0]}",
                    f"or /{BotCommands.YtdlZipLeechCommand[1]} Leech yt-dlp support link as zip",
                ),
                ...
                (
                    f"{BotCommands.CategorySelect}",
                    "Select category to upload only mirror",
                ),
                (f"{BotCommands.CancelMirror}", "Cancel a Task"),
                (
                    f"{BotCommands.CancelAllCommand[0]}",
                    f"Cancel all tasks which added by you or {BotCommands.CancelAllCommand[1]} to in bots.",
                ),
