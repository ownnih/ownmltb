...
def sendStatusMessage(msg, bot):
    ...
    with status_reply_dict_lock:
        ...
        if not Interval:
            Interval.append(
                setInterval(
                    config_dict["DOWNLOAD_STATUS_UPDATE_INTERVAL"], update_all_messages
                )
            )
...
def sendDmMessage(bot, message, dmMode, isLeech=False):
    ...
    try:
        return bot.sendMessage(
            message.from_user.id, disable_notification=True, text=message.link
        )
    except RetryAfter as r:
        LOGGER.warning(str(r))
        sleep(r.retry_after * 1.5)
        ...
        sendMessage(
            "<b>You didn't START the bot in DM</b>", bot, message, buttons.build_menu(1)
        )
...
def sendLogMessage(bot, message, link, tag):
    ...
    try:
        ...
        ) or "https://api.telegram.org/file/" in link:
...
def chat_restrict(message):
    ...
    if not isAdmin(message):
        message.chat.restrict_member(
            message.from_user.id, ChatPermissions(), int(time() + 60)
        )
...
def anno_checker(message):
    ...
    sendMessage(
        f"{_msg} Verification\nIf you hit Verify! Your username and id will expose in bot logs!",
        message.bot,
        message,
