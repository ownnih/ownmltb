            ...
            if len(self.sameDir) > 1:
                ...
                for subdir in listdir(f"{self.dir}/{folder_name}"):
                    ...
                    if subdir in listdir(des_path):
                        sub_path = rename(
                            sub_path, f"{self.dir}/{folder_name}/1-{subdir}"
                        )
        ...
        if self.isZip:
            ...
            if self.pswd:
                if self.isLeech and int(size) > LEECH_SPLIT_SIZE:
                    ...
                    self.suproc = Popen(
                        ["7z", "a", "-mx=0", f"-p{self.pswd}", path_, m_path]
                    )
            elif self.isLeech and int(size) > LEECH_SPLIT_SIZE:
                LOGGER.info(f"Zip: orig_path: {m_path}, zip_path: {path_}.0*")
                self.suproc = Popen(
                    ["7z", f"-v{LEECH_SPLIT_SIZE}b", "a", "-mx=0", path_, m_path]
                )
            else:
                LOGGER.info(f"Zip: orig_path: {m_path}, zip_path: {path_}")
            ...
            try:
                ...
                if path.isdir(m_path):
                    ...
                    for dirpath, _, files in walk(m_path, topdown=False):
                        for file_ in files:
                            if search(
                                "\.part0*1\.rar$|\.7z\.0*1$|\.zip\.0*1$|\.zip$|\.7z$|^.(?!.*\.part\d+\.rar)(?=.*\.rar$)",
                                file_,
                            ):
                    ...
                    if self.pswd:
                        self.suproc = Popen(
                            ["7z", "x", f"-p{self.pswd}", m_path, f"-o{path_}", "-aot"]
                        )
                    else:
            ...
            if not files:
                ...
                for index, (link, name) in enumerate(files.items(), start=1):
                    ...
                    if len(fmsg.encode() + msg.encode()) > 4000:
                        ...
                        sendMessage(
                            msg + fmsg, self.bot, self.message, buttons.build_menu(2)
                        )
                        sleep(1)
                        fmsg = ""
                if fmsg != "":
