...
def start(update, context):
    if config_dict["DM_MODE"]:
    ...
    elif "private" in update.effective_chat.type:
        buttons = ButtonMaker()
        buttons.buildbutton("Main Group", "https://t.me/DeltaX_Universe")
        reply_markup = buttons.build_menu(1)
        pic = "https://telegra.ph/file/64819dd33fd0b13212085.jpg"

        return sendPhoto(
            "Hey you!!\nJoin to our universe",
            context.bot,
            update.message,
            pic,
            reply_markup,
        ...
        sendMessage("You What's up!!!", context.bot, update.message)
                    ...
                    for index, link in enumerate(links, start=1):
                        ...
                        if len(msg.encode()) > 4000:
                            if "Restarted Successfully!" in msg and cid == chat_id:
                                try:
                                    bot.editMessageText(
                                        "Restarted Successfully!", chat_id, msg_id
                                    )
                                    bot.sendMessage(
                                        chat_id, msg, reply_to_message_id=msg_id
                                    )
                                except:
                                    pass
                                remove(".restartmsg")
